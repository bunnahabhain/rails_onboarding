...
Headless API support for mobile apps and single-page applications.

### Features

- **JSON API**: RESTful JSON endpoints
- **Token Authentication**: Support for API tokens
- **Serialization**: Flexible serialization options
  ...
  namespace :api do
    namespace :v1 do
      resources :onboarding, only: [] do
        ...
        member do
          post 'steps/:step_name/complete', to: 'onboarding#complete_step'
          post 'steps/:step_name/skip', to: 'onboarding#skip_step'
        end
      end

      resources :tooltips, only: [:index] do
        member do
          post :dismiss
        end
      end
...
3. **Use API Controller**:

```ruby
# app/controllers/api/v1/onboarding_controller.rb
module Api
  module V1
    class OnboardingController < RailsOnboarding::ApiController
      # Inherits all API functionality
    end
  end
end
```
...
```bash
POST /api/v1/onboarding/steps/welcome/complete
Authorization: Bearer YOUR_TOKEN
...
```bash
POST /api/v1/onboarding/steps/explore/skip
Authorization: Bearer YOUR_TOKEN
...
Response:
{
  "success": true,
  "data": {
    "message": "Step 'explore' skipped successfully",
...
```bash
GET /api/v1/onboarding/tooltips
Authorization: Bearer YOUR_TOKEN
...
```ruby
# Include API mode in your own controller
class MyApiController < ApplicationController
  include RailsOnboarding::ApiMode

  enable_api_mode version: 'v2', authentication: :bearer

  def custom_endpoint
    render_api_success({ custom: 'data' })
  end
end
```
...
**Solution**: Implement token authentication in your User model:
