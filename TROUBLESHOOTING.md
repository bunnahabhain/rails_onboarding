...
1. Verify API token exists:
   ...
   rails console
   user = User.find(1)
   user.api_token  # Should return token string
   ```
...
4. Test authentication:
   ```bash
   rails console
   User.find_by_api_token('your_token_here')
   ```
...
**Error:**
```
ActionController::RoutingError: No route matches [POST] "/api/v1/onboarding/complete_step"
```
